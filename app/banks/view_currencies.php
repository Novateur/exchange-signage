...
<body class="" style="font-family: verdana">
  ...
  <script>
      ...
      function delete_currencies_personal(id)
      {
        $('#paste_proceed').html("<button type='button' class='btn btn-danger' onclick=\"cont_del_currency_personal("+id+")\">Ok</button>");
        $('#delete_modal').modal('show');
      }

      function cont_del_currency_personal(id)
      {
        $.post("handler/delete_currency_personal.php",{id:id},function(response){
          //alert("Request sent");
          if(response=="deleted")
          {
